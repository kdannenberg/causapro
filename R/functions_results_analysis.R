...
compare_effects_per_position <- function (results,
  ...
  if (missing(subset_of_positions)) {
    # subset_of_positions = c(1:dim(results$all_pairwise_effects)[2])
    subset_of_positions = colnames(all_pairwise_effects)
  }


  if (!all(subset_of_positions %in% rownames(all_pairwise_effects))) {
    # for each name (e.g. of a column in all_pairwise_effects), find the index of the first
    # rowname of all_pairwise_effects that contains this name
    f_name <- function(name) {
      return(Position(function_set_parameters(grepl, parameters = list(pattern = name)), rownames(all_pairwise_effects)))
    }
    subset_of_positions <- sapply(colnames(all_pairwise_effects), f_name)
  }

  all_pairwise_effects <- all_pairwise_effects[subset_of_positions, ]
  rownames(all_pairwise_effects) <- colnames(all_pairwise_effects)
  ...
  for (format in output_formats) {
    if (!nchar(outpath) == 0) {
      ...
      if (heatmap) {
        ...
        heatmap3(all_pairwise_effects, revC = TRUE, symm = FALSE,
        ...
        heatmap3(all_pairwise_effects, revC = TRUE, symm = FALSE, trace = "none",
                 col = heat.colors(length(palette.breaks) - 1), #breaks = palette.breaks,
                 # dendrogram = "col", Rowv = FALSE,
                 method = "ward.D",
        ...
        heatmap3(all_pairwise_effects, revC = FALSE, symm = FALSE,
                 col = heat.colors(length(palette.breaks) - 1), #breaks = palette.breaks,
